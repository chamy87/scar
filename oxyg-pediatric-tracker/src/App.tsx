...
import { createReading, deleteReading, getReadings, updateReading, buildReportSummary } from "@/lib/api"
...
function AppShell({ children }: { children: React.ReactNode }) { return <div className="min-h-screen bg-bg-clinical text-text-main">{children}</div> }
function Sidebar() { return <aside className="no-print w-52 border-r border-border-soft bg-white p-4"><h1 className="mb-6 text-lg font-semibold text-scarlet-deep">OxyG Pediatric Tracker</h1><nav className="space-y-2">{links.map((l) => <Link key={l} to={l} className="block rounded-md px-3 py-2 hover:bg-scarlet-soft">{l.replace("/", "")}</Link>)}</nav></aside> }
function AuthStatus({ isAuthenticated, email }: { isAuthenticated: boolean; email?: string }) { return <span className="text-sm text-text-muted">{isAuthenticated ? email : "Public Read-Only"}</span> }
function TopNav({ isAuthenticated, email, onLogin, onLogout }: { isAuthenticated: boolean; email?: string; onLogin: () => void; onLogout: () => void }) {
  return <header className="no-print flex items-center justify-between border-b border-border-soft bg-white px-6 py-3"><AuthStatus isAuthenticated={isAuthenticated} email={email} />{isAuthenticated ? <Button onClick={onLogout}><LogOut className="mr-2 inline size-4" />Logout</Button> : <Button onClick={onLogin}><LogIn className="mr-2 inline size-4" />Login</Button>}</header>
}
...
function ReadingsTable({ readings, isAuthenticated, onEdit, onDelete }: { readings: Reading[]; isAuthenticated: boolean; onEdit: (r: Reading) => void; onDelete: (id: string) => void }) {
  return <table className="w-full rounded-md border border-border-soft bg-white text-sm"><thead><tr className="bg-scarlet-soft text-left"><th className="p-2">Time</th><th>SpO2</th><th>BPM</th><th>PI</th><th className="no-print">Actions</th></tr></thead><tbody>{readings.map((r) => <tr key={r.id} className="border-t border-border-soft"><td className="p-2">{new Date(r.recorded_at).toLocaleString()}</td><td>{r.spo2}</td><td>{r.bpm}</td><td>{r.pi ?? "-"}</td><td className="no-print">{isAuthenticated && <div className="flex gap-2"><Button className="bg-accent-rose" onClick={() => onEdit(r)}>Edit</Button><Button className="bg-scarlet-deep" onClick={() => onDelete(r.id)}>Delete</Button></div>}</td></tr>)}</tbody></table>
}
...
function LoginForm() {
  ...
  return <div className="mx-auto mt-16 max-w-md rounded-md border border-border-soft bg-white p-6"><h2 className="mb-1 text-2xl font-semibold text-scarlet-deep">OxyG Pediatric Tracker</h2><p className="mb-4 text-sm text-text-muted">Supabase Auth Sign In</p><form className="space-y-3" onSubmit={submit}><input className="w-full rounded border border-border-soft p-2" type="email" placeholder="Email" value={email} onChange={(x) => setEmail(x.target.value)} required /><input className="w-full rounded border border-border-soft p-2" type="password" placeholder="Password" value={password} onChange={(x) => setPassword(x.target.value)} required />{error && <p className="text-sm text-accent-rose">{error}</p>}<Button className="w-full" disabled={submitting} type="submit">{submitting ? "Signing in..." : "Login"}</Button></form><Link to="/dashboard" className="mt-3 block text-center text-sm text-scarlet-primary hover:underline">Back to dashboard</Link></div>
}
...
export function App() {
  ...
  const handleLogout = async () => { await signOut(); navigate("/dashboard") }
  const email = user?.email ?? undefined

  return <AppShell><div className="flex"><Sidebar /><main className="flex-1"><TopNav isAuthenticated={isAuthenticated} email={email} onLogin={() => navigate("/login")} onLogout={handleLogout} /><div className="space-y-4 p-6"><SafetyDisclaimer />{loading && <div className="rounded-md border border-border-soft bg-white p-3 text-sm text-text-muted">Loading session...</div>}{!hasSupabaseEnv && <div className="rounded-md border border-accent-rose bg-white p-3 text-sm text-accent-rose">Missing Supabase environment variables. Set <code>VITE_SUPABASE_URL</code> and <code>VITE_SUPABASE_ANON_KEY</code>.</div>}{dataError && <div className="rounded-md border border-accent-rose bg-white p-3 text-sm text-accent-rose">Database setup issue: {dataError}. Run the SQL in <code>supabase/migration.sql</code> then <code>supabase/seed.sql</code>.</div>}<Routes>
    <Route path="/" element={<Navigate to="/dashboard" />} />
    <Route path="/dashboard" element={<><DashboardHeader /><div className="grid gap-3 md:grid-cols-4"><MetricCard title="Avg SpO2" value={avg.averageSpo2.toFixed(1)} icon={<Droplets className="size-4" />} /><MetricCard title="Avg BPM" value={avg.averageBpm.toFixed(1)} icon={<HeartPulse className="size-4" />} /><MetricCard title="Readings" value={`${avg.count}`} icon={<Activity className="size-4" />} /><MetricCard title="Flags" value={`${avg.thresholdFlags}`} icon={<span className="rounded-full bg-scarlet-soft px-3 py-1 text-xs font-medium text-scarlet-deep">Review</span>} /></div><TrendChart data={filtered} /></>} />
    <Route path="/readings" element={<><div className="flex items-center justify-between"><h2 className="text-xl font-semibold">Readings</h2>{isAuthenticated && <Button onClick={() => { setEditing(null); setModalOpen(true) }}>Add Reading</Button>}</div><ReadingsTable readings={readings} isAuthenticated={isAuthenticated} onEdit={(r) => { setEditing(r); setModalOpen(true) }} onDelete={async (id) => { try { await deleteReading(id); await fetchAll() } catch (error) { setDataError(error instanceof Error ? error.message : "Delete failed") } }} /><ReadingFormModal open={modalOpen} onClose={() => setModalOpen(false)} initial={editing} onSave={async (payload) => { try { if (editing) await updateReading(editing.id, payload); else if (user?.id) await createReading(payload, user.id); setModalOpen(false); await fetchAll() } catch (error) { setDataError(error instanceof Error ? error.message : "Save failed") } }} /></>} />
